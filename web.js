...
async function Init(){
    ...
    weekData = water
    water.getPercent(weekData.Valueobject)
    let yfunc = await water.initWater(12)
    yearData = water
    water.getPercent(yearData.Valueobject)
}
//비디오파일 저장 위치
//비디오를 저장할때 필요한 코드입니다.
...
app.post('/deletevideo', function (req, res, next) {
    try {
        mirrorsave.deletefile(req.body.name, "video", "None",0)
    } catch (err) {
        console.dir(err.stack);
    }
...
app.post('/deleteimage', function (req, res, next) {
    try {
        mirrorsave.deletefile(req.body.name, "image", "None",0)
    } catch (err) {
        console.dir(err.stack);
    }
    res.redirect('mediacontents')
...
app.post('/deletereservationvideo', function (req, res, next) {
    try {
        mirrorsave.deletefile(req.body.name, "video", "reservation",0)
    } catch (err) {
        console.dir(err.stack);
    }
...
app.post('/deletereservationimage', function (req, res, next) {
    try {
        mirrorsave.deletefile(req.body.name, "image", "reservation",0)
    } catch (err) {
        console.dir(err.stack);
    }
    res.redirect('bookmedia')
...
app.get('/wateruseage', function (req, res) {
    Water.find(function (err, data) {
        MonthUseage.find(function (err, yeardata) {
            const currentMonth = parseInt(getDays.month)
            let lastMonth = 0
            //저번달 총,이번달 총 사용량 구하기
            //이번달이 1월이면 저번달 총사용량은 0으로 초기화
            if ((Valueyeardata[currentMonth-2] - 2) < 0) {
                lastMonth = 0
            }
...
app.get('/wateruseage', function (req, res) {
            ...
            res.render('wateruseage', {
                ...
                cbweekData : Math.max.apply(null,weekData[0]),cbyearData : Math.max.apply(null,yearData[0])
            })
})
