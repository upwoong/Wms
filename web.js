        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].Date == date && data[i].type == "reservation") {
                ...
                changefile.save(function (err, slience) {
                    ...
                    return console.log("매일 오전6시 예약 이미지파일 업데이트 완료")
                })
                imagestate = true
            }
        }
            ...
            for (let i = 0; i < data.length; i++) {
                if (data[i].type == "None") {
                    ...
                    changefile.save(function (err, slience) {
                        ...
                        return console.log("매일 오전6시 기본 이미지파일 업데이트 완료")
                    })
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].Date == date && data[i].type == "reservation") {
                ...
                changefile.save(function (err, slience) {
                    ...
                    return console.log("매일 오전6시 예약 비디오파일 업데이트 완료")
                })
                videostate = true
            }
        }
            ...
            for (let i = 0; i < data.length; i++) {
                if (data[i].type == "None") {
                    ...
                    changefile.save(function (err, slience) {
                        ...
                        return console.log("매일 오전6시 기본 비디오파일 업데이트 완료")
                    })
        ...
        if (localselect == firstSheet["C" + index].v && cityselect == firstSheet["D" + index].v && villageselect == firstSheet["E" + index].v) {
            ...
            if (data != "") {
                ...
                weather.save(function (err, slience) {
                    ...
                    return console.log("지역 선택 업데이트 완료")
                })
            }
        }
    }
...
app.get('/dummy', function (req, res) {
    fs.unlink(`/home/hosting_users/creativethon/apps/creativethon_wmsadmina/smartmirror/image/sub_31636620543350.png`, function (err) {
        if (err) console.log(err)
    })
    res.render('dummy')
})
